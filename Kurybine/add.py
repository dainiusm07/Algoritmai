...
def rectangle3Drot(c,e,RGB,phi): # c - center, e - width of edges, RGB - color
  global vertices, faces
  F = [[0,4,5,1],[0,1,3,2],[0,2,6,4],[1,5,7,3],[2,3,7,6],[4,6,7,5]]
  V = [[0,0,0],[0,0,e[2]],[0,e[1],0],[0,e[1],e[2]],[e[0],0,0],[e[0],0,e[2]],[e[0],e[1],0],[e[0],e[1],e[2]]] 
  for i in range (0,6):
  ...
  for j in range (0,8):
    p1 = V[j][0]-e[0]/2
    p2 = c[1]+V[j][1]-e[1]/2 # lieka savo vietoje
    p3 = V[j][2]-e[2]/2
    s1 = c[0]+p1*math.cos(phi)-p3*math.sin(phi)
    s2 = c[2]+p1*math.sin(phi)+p3*math.cos(phi)
    vertices += [str(s1)+' '+str(p2)+' '+str(s2)]
